...
public class PlayerInventoryManagerPatch {
    @ModMethodPatch(target = PlayerInventoryManager.class, name = "removeItems", arguments = {
            Item.class, int.class, boolean.class, boolean.class, boolean.class, String.class})
    public static class RemoveItemsPatch {
        @Advice.OnMethodEnter
        public static void onEnter(@Advice.AllArguments(readOnly = false,
                typing = Typing.DYNAMIC) Object[] arguments) {
            if (!"travelscroll".equals(arguments[5])) {
                return;
            }
